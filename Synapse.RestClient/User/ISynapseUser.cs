...
namespace Synapse.RestClient.User
{
	...
	public interface ISynapseUserApiClient
	{
		...
		Task<AddDocsResponse> AddDocsAsync(AddDocsRequest req);
	...
	public class SynapseUserApiClient : ISynapseUserApiClient
	{
		...
		public async Task<AddDocsResponse> AddDocsAsync(AddDocsRequest msg)
		{
			var req = new RestRequest("user/docs/add", Method.POST);
			var body = new
			{
				login = new
				{
				...
				user = new
				{
					documents = new[] {
						new
						{
							...
							day = msg.DateOfBirth.Day,
							month = msg.DateOfBirth.Month,
							year = msg.DateOfBirth.Year,
							address_street = (msg.Address1 + " " + msg.Address2).Trim(),
							address_city = msg.City,
							address_subdivision = msg.State,
							address_postal_code = msg.PostalCode,
							address_country_code = msg.CountryCode,
							virtual_docs = new[] {
								new {
							...
							physical_docs = new[] {
								new {
									document_value = msg.PhysicalDocumentValue,
									document_type = ToString(msg.PhysicalDocumentType)
								}
							}
						}
					},
		...
		private static string ToString(SynapseDocumentType docType)
		{
			...
			else if (docType == SynapseDocumentType.GovtId)
			{
				return "GOVT_ID";
			}
			else if (docType == SynapseDocumentType.GovtIdInt)
			{
				return "GOVT_ID_INT";
			}
